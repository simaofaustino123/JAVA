...
public class Conjunto {
	...
	public void adiciona(String nome) {
		...
		if(!contem(nome)) {
			...
			System.out.println("O nome " + nome.toUpperCase() + " já está na lista");
		}
	}
	...
	private boolean contem(String nome) {
		...
		int indice = calculaIndice(nome);
		// 2 - Busca o nome  na tabela específica do índice
		return tabela.get(indice).contains(nome);
	}
	
	public void remove(String nome) {
		//1 - Verifica se o nome está em alguma lista
		if(contem(nome)) {
			// 2 - Caso sim, calcula o índice do nome
			int indice = calculaIndice(nome);
